...
typedef struct FrameThreadContext {
    ...
    volatile int die;              ///< Set when threads should exit.
} FrameThreadContext;
...
static attribute_align_arg void *frame_worker_thread(void *arg)
{
    ...
    while (1) {
        if (avpriv_atomic_int_get(&p->state) == STATE_INPUT_READY &&
            !avpriv_atomic_int_get(&fctx->die)) {
            pthread_mutex_lock(&p->mutex);
            while (avpriv_atomic_int_get(&p->state) == STATE_INPUT_READY &&
                   !avpriv_atomic_int_get(&fctx->die))
                pthread_cond_wait(&p->input_cond, &p->mutex);
            pthread_mutex_unlock(&p->mutex);
        }

        if (avpriv_atomic_int_get(&fctx->die))
            break;
        ...
        if (avpriv_atomic_int_get(&p->state) == STATE_SETTING_UP)
            ff_thread_finish_setup(avctx);

        avpriv_atomic_int_set(&p->state, STATE_INPUT_READY);
...
static int submit_packet(PerThreadContext *p, AVPacket *avpkt)
{
    ...
    if (prev_thread) {
        int err;
        if (avpriv_atomic_int_get(&prev_thread->state) == STATE_SETTING_UP) {
            pthread_mutex_lock(&prev_thread->progress_mutex);
            while (avpriv_atomic_int_get(&prev_thread->state) == STATE_SETTING_UP)
    ...
    avpriv_atomic_int_set(&p->state, STATE_SETTING_UP);
    ...
    if (!p->avctx->thread_safe_callbacks && (
        ...
        int state = avpriv_atomic_int_get(&p->state);
        while (state != STATE_SETUP_FINISHED && state != STATE_INPUT_READY) {
            pthread_mutex_lock(&p->progress_mutex);
            while (avpriv_atomic_int_get(&p->state) == STATE_SETTING_UP)
                pthread_cond_wait(&p->progress_cond, &p->progress_mutex);

            if (avpriv_atomic_int_get(&p->state) == STATE_GET_BUFFER) {
                p->result = ff_get_buffer(p->avctx, p->requested_frame, p->requested_flags);
                avpriv_atomic_int_set(&p->state, STATE_SETTING_UP);
                pthread_cond_signal(&p->progress_cond);
            }
            pthread_mutex_unlock(&p->progress_mutex);
            state = avpriv_atomic_int_get(&p->state);
        }
    }
...
void ff_thread_finish_setup(AVCodecContext *avctx) {
    ...
    pthread_mutex_lock(&p->progress_mutex);
    avpriv_atomic_int_set(&p->state, STATE_SETUP_FINISHED);
    pthread_cond_broadcast(&p->progress_cond);
    pthread_mutex_unlock(&p->progress_mutex);
}
...
static void frame_thread_free(AVCodecContext *avctx, int thread_count)
{
    ...
    avpriv_atomic_int_set(&fctx->die, 1);
...
int ff_thread_get_buffer(AVCodecContext *avctx, ThreadFrame *f, int flags)
{
    ...
    if (avctx->thread_safe_callbacks || (
        ...
        avpriv_atomic_int_set(&p->state, STATE_GET_BUFFER);
        pthread_mutex_lock(&p->progress_mutex);
        pthread_cond_signal(&p->progress_cond);
